...
import {configure, Vector4, Matrix4, Matrix3, Quaternion, equals} from '@math.gl/core';

test('Vector4#import', () => {
  expect(typeof Vector4).toBe('function');
});

test('Vector4#construct and Array.isArray check', () => {
...
test('Vector4#debug validators', () => {
  ...
  expect(() => new Vector4(NaN, 0, 0, 1)).toThrow();
  expect(() => new Vector4().copy([NaN, 0, 0, 1])).toThrow();
  expect(() => new Vector4().copy([0, 0, 0])).toThrow();
  configure({debug: false});
  expect(() => new Vector4(NaN, 0, 0, 1)).not.toThrow();
  expect(() => new Vector4().copy([NaN, 0, 0, 1])).not.toThrow();
  configure({debug});
});

test('Vector4#from', () => {
  expect(equals(new Vector4().from([1, 2, 3, 4]), [1, 2, 3, 4])).toBe(true);
  expect(equals(new Vector4().from({x: 1, y: 2, z: 3, w: 4}), [1, 2, 3, 4])).toBe(true);
});

test('Vector4#to', () => {
  const vector4 = new Vector4(1, 2, 3, 4);
  expect(equals(vector4.to([0, 0, 0, 0]), [1, 2, 3, 4])).toBe(true);
  expect(vector4.to({x: 0, y: 0, z: 0, w: 0})).toEqual({x: 1, y: 2, z: 3, w: 4});
});
...
test('Vector4#toString', () => {
  ...
  for (const testCase of TEST_CASES) {
    ...
    expect(String(v)).toBe(testCase.string);
    expect(`${v}`).toBe(testCase.string);
  }
});

test.skip('Vector4#scale', () => {
  const TEST_CASES = [
...
test('Vector4#transformByMatrix3', () => {
...
test('Vector4#transformByMatrix2', () => {
...
test('Vector4#transformByQuaternion', () => {
